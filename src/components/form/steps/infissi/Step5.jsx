...
export default function StepInfissi({ onNext, onBack, formData }) {
  const [servizio5, setServizio5] = useState(formData.servizio5 || '');
  ...
  return (
    <div className="space-y-6 bg-white mx-auto h-[450px] flex flex-col justify-between">
      <div className="flex flex-col gap-4">
        <h2 className='text-2xl font-semibold text-gray-800'>Di che materiale saranno le finestre? </h2>
        {['PVC', 'Legno', 'Alluminio', 'Vorrei un vostro consiglio'].map((option) => (
          <label key={option} className={`flex items-center gap-3 p-3 rounded-lg cursor-pointer transition-colors duration-300 ${
            servizio5 === option ? 'bg-green-100 border-2 border-green-500' : 'bg-gray-50 border-2 border-gray-200'
          } hover:bg-green-50`}>
            <input
              type="radio"
              name="servizio"
              value={option}
              checked={servizio5 === option}
              onChange={() => {setServizio5(option);  setError('');}}
